...
func TestFindCustomer_Success(t *testing.T) {
	...
	customer, err := Find(db, "barbearia-test", 1, 10, "owner")
	if err != nil {
...
func TestFindCustomer_NotFound(t *testing.T) {
	...
	_, err := Find(db, "barbearia-test", 999, 10, "owner")
...
func TestFindCustomer_WrongOrg(t *testing.T) {
	...
	_, err := Find(db, "barbearia-test", 3, 10, "owner")
	if !errors.Is(err, ErrCustomerNotFound) {
		t.Errorf("expected ErrCustomerNotFound, got: %v", err)
	}
}

// TestFindCustomer_Forbidden verifica que usuário comum recebe ErrForbidden.
